...
namespace AliaSQL.Core.Services.Impl
{
  public class ConnectionStringGenerator : IConnectionStringGenerator
  {
    ...
    public ConnectionSettings GetConnectionSettings(string connectionString, TimeSpan scriptTimeout)
    {
      var cs = new SqlConnectionStringBuilder(connectionString);
      return new ConnectionSettings(cs.DataSource, cs.InitialCatalog, cs.IntegratedSecurity, cs.UserID, cs.Password, scriptTimeout);
    }
  }
}